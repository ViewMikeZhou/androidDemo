...
public class XyzRuler extends View implements ValueAnimator.AnimatorListener {
    ...
    private void drawScale(Canvas canvas) {
        ...
        for (int x = 0; x < mWidth; x++) {
            ...
            if ((-sumMoveX + x) % (pixel * 6) == 0) {
                y += lineHeight;
            }
            if ((-sumMoveX + x) % (pixel * 12) == 0) {
                y += lineHeight;
                isDrawText = true;
            }
            ...
            if (isDrawText) {
                if (text >= begin && text <= end) {
                    Log.e("test",text+"");
                    canvas.drawText(String.valueOf(text/12)+":0", x, isTop ? y + lineToText : mHeight - y - lineToText, tPaint);
                }
            }
        }
    }
    ...
    public static class Build{
     ...
     /*   private float borderWidth;      //边框宽度
        ...
        public Build(){
            mXyzRuler = new XyzRuler(App.getInstance());
        }

        public Build setBorderWidth(float borderWidth){
            mXyzRuler.borderWidth =borderWidth;
            return this;
        }

        public Build setLineWidth(float lineWidth){
            mXyzRuler.lineWidth = lineWidth;
            return this;
        }

        public Build setAnimTime(long animTime){
            mXyzRuler.animTime = animTime;
            return this;
        }

        public Build setBorderColor(int borderColor){
            mXyzRuler.borderColor = borderColor;
            return this;
        }
        public Build setLineColor(int lineColor){
            mXyzRuler.lineColor = lineColor;
            return  this ;
        }


        public Build setTrigonSize(int trigonSize){
            mXyzRuler.trigonSize =trigonSize;
            return this;
        }

        public Build setLineHeigt(int lineHeight){
            mXyzRuler.lineHeight = lineHeight;
            return this;
        }
        public Build setLinTotext(int linTotext){
            mXyzRuler.lineToText = linTotext;
            return this;
        }

        public  Build setBegin(int begin){
            mXyzRuler.begin = begin;
            return this;
        }
        ...
        public Build setMinVelocity(int minVelocity){
            mXyzRuler.minVelocity = minVelocity;
            return this;
        }
        public  Build setIsRect (boolean isRect){
            mXyzRuler.isRect = isRect;
            return this;
        }

        public  Build setIstop(boolean istop){
            mXyzRuler.isTop = istop;
            return  this;
        }

        public XyzRuler build(){
            return mXyzRuler== null? new XyzRuler(App.getInstance()) : mXyzRuler ;
        }
